...
public interface TenantRepository extends JpaRepository<Tenant, Integer>{
	...
	@Query("-	select 1.0*(select count(r1) from Request r1 where r1.status='ACCEPTED' and r1.tenant=r)/r.requests.size from Tenant r")
	Double findAvgAcceptedRequestPerTenant();
	
	@Query("-	select 1.0*(select count(r1) from Request r1 where r1.status='DENIED' and r1.tenant=r)/r.requests.size from Tenant r")
	...
	Tenant findTenantMoreApprovedRequest();
	...
	Tenant findTenantMoreDeniedRequest();
	...
	Tenant findTenantMorePendingRequest();
